...
constexpr char kDefaultTracingHostname[] = "hostname";
...
void Server::Start() {
  ...
  if (options_.enable_tracing) {
    ...
    if (hostname == nullptr) hostname = kDefaultTracingHostname;
    std::stringstream opencensus_task;
    opencensus_task << options_.tracing_task_name << "-" << getpid() << "@" << hostname;
    opencensus::exporters::trace::StackdriverExporter::Register(options_.tracing_gcp_project_id);
    opencensus::trace::TraceConfig::SetCurrentTraceParams({
    	kDefaultTracingMaxAttributes,
		kDefaultTracingMaxAnnotations,
		kDefaultTracingMaxMessageEvents,
		kDefaultTracingMaxLinks,
		opencensus::trace::ProbabilitySampler(options_.tracing_sampler_probability)
    });
  }
#endif
